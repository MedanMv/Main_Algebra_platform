...
function toggleSession(classUUID) {
    ...
    .then(data => {
        if (data.success) {
            ...
            const sessionBtn = document.getElementById('sessionBtn'); // Locate the button
            if (sessionBtn) {
                if (data.new_session_active === 1) {
                    // Session is active: Update to lightgreen background with black text
                    sessionBtn.style.backgroundColor = 'lightgreen';
                    sessionBtn.style.color = 'black';
                } else if (data.new_session_active === 0) {
                    // Session is inactive: Update to red background with white text
                    sessionBtn.style.backgroundColor = 'red';
                    sessionBtn.style.color = 'white';
                } else {
                    console.error(`Unexpected new_session_active value: ${data.new_session_active}`);
                }
            } else {
                console.error('Session button not found in the DOM.');
            }
        } else {
...
const eventSource = new EventSource('/live_notifications');
eventSource.onmessage = function(event) {
    console.log("New notification:", event.data);
    alert(event.data); // Optional: Display the notification
};
...
function openClassInterface(className, classUUID) {//BODYAGA suda dobavish div s otobrageniem className no classUUID ne trogai!!!!!
    ...
    content.innerHTML = `
        ...
        <div class="class-head">
            ...
            <div class="nest-class">
                <input type="text" class="exit-log-input" id="readonlyInput" name="readonlyInput" value="" readonly>
                <button class="session-btn" id="sessionBtn" onclick ="toggleSession('${classUUID}')">Toggle session</button>
            </div>
        </div>
        <br>
...
<h2>Grade 7 <input type="checkbox" id="grade-check-7" onclick="toggleGrade(7, '${classUUID}')"></h2>
<table>
<tr><th>#</th><th>Theme</th><th>Formula</th><th>Check</th></tr>
...
<h2>Grade 8 <input type="checkbox" id="grade-check-8" onclick="toggleGrade(8, '${classUUID}')"></h2>
<table>
<tr><th>#</th><th>Theme</th><th>Formula</th><th>Check</th></tr>
...
<h2>Grade 9 <input type="checkbox" id="grade-check-9" onclick="toggleGrade(9, '${classUUID}')"></h2>
<table>
<tr><th>#</th><th>Theme</th><th>Formula</th><th>Check</th></tr>
...
<h2>Grade 10 <input type="checkbox" id="grade-check-10" onclick="toggleGrade(10, '${classUUID}')"></h2>
<table>
<tr><th>#</th><th>Theme</th><th>Formula</th><th>Check</th></tr>
...
<h2>Grade 11 <input type="checkbox" id="grade-check-11" onclick="toggleGrade(11, '${classUUID}')"></h2>
<table>
<tr><th>#</th><th>Theme</th><th>Formula</th><th>Check</th></tr>
...
<tr><td>Index</td><td>Theme</td><td>Formula</td><td><input type="checkbox" class="formula-check-Index grade-Class"></td></tr>
    ...
    `;    
    fetch(`/get_session_active?class_uuid=${classUUID}`)
    .then(response => response.json())
    .then(data => {
        console.log('Fetched session_active:', data.session_active); // Debug fetched data
        const sessionBtn = document.getElementById('sessionBtn');
        if (sessionBtn) {      
            if (data.session_active === 1) {
            // Session is active: Set button color to light green and text to black
                sessionBtn.style.backgroundColor = 'lightgreen';
                sessionBtn.style.color = 'black';
            } else if (data.session_active === 0) {
            // Session is inactive: Set button color to red and text to white
                sessionBtn.style.backgroundColor = 'red';
                sessionBtn.style.color = 'white';
            }
        } else {
    ...
    .catch(error => console.error('Error fetching session_active:', error));
